...
export function createParsedOutput(): ParsedOutput {
  return { filesEdited: [], summary: '', currentAction: '', toolUseCount: 0 };
}
...
export function updateParsedOutput(parsed: ParsedOutput, line: string): ParsedOutput {
  try {
    ...
    // Assistant text messages → extract summary (last non-code paragraph)
    if (event.type === 'assistant' && event.message?.content) {
      ...
      for (const block of contents) {
        if (block.type === 'text' && typeof block.text === 'string') {
          ...
          // Extract last paragraph that isn't a code block
